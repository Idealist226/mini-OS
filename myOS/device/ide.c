...
/* 定义可读写的最大扇区数，调试用的 */
#define max_lba ((80*1024*1024/512) - 1)    // 只支持 80M 硬盘
...
static void select_sector(struct disk* hd, uint32_t lba, uint8_t sec_cnt) {
    ...
    /* 写入 lba 地址（即扇区号）*/
    outb(reg_lba_l(channel), lba);          // lba 地址的低 8 位，不用单独取出低 8 位，outb 函数中的汇编指令 outb %b0, %w1 会只用 al
...
static void read_from_sector(struct disk* hd, void* buf, uint8_t sec_cnt) {
    ...
    if (sec_cnt == 0) {
        /* 因为 sec_cnt 是 8 位变量，由主调函数将其赋值时，若为 256 则会将最高位的1 丢掉变为 0 */
...
static bool busy_wait(struct disk* hd) {
    struct ide_channel* channel = hd->my_channel;
    uint16_t time_limit = 30 * 1000;    // 可以等待 3000 毫秒
