...
//一页大小的位图可表示128MB内存，位图位置安排在地址0xc009a000，这样本系统最大支持4个页框的位图，即512MB
...
static void page_table_add(void* _vaddr, void* _page_phyaddr) {
    ...
    //先在页目录内判断页目录项的 P 位，若为 1，则表示该表已存在
    if(*pde & 0x00000001) {
        //页目录项和页表项的第 0 位为 P，此处判断目录项是否存在
        ASSERT(!(*pte & 0x00000001));   //此时 pte 应该不存在
        if(!(*pte & 0x00000001)) {      //只要是创建页表，pte 就应该不存在，多判断一下放心
            *pte = (page_phyaddr | PG_US_U | PG_RW_W | PG_P_1);     //创建 pte
        } else {                        //目前执行不到这里
            PANIC("pte repeat");
            *pte = (page_phyaddr | PG_US_U | PG_RW_W | PG_P_1);
        }
    } else {
        //页目录项不在，所以要先创建页目录再创建页表项
        //页表中用到的页框一律从内核空间分配
...
static void* malloc_page(enum pool_flags pf, uint32_t pg_cnt) {
    ...
    //因为虚拟地址是连续的，耽误离地值可以是不连续的，所以逐个做映射
    while(cnt-- > 0) {
        ...
        page_table_add((void*)vaddr, page_phyaddr);     //在表中做映射
        vaddr += PG_SIZE;                               //下一个虚拟页
    }
    return vaddr_start;
}

/* 从内核物理内存池中申请内存，成功返回虚拟地址，失败返回 NULL */
...
static void mem_pool_init(uint32_t all_mem) {
    ...
    //当前已经使用的内存字节数，1M部分已经使用了，1M往上是页表所占用的空间
